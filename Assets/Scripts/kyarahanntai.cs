...
public class kyarahanntai : MonoBehaviour {
	bool once;
	int mabatakiflag;
	bool goal;
	int hanntoumeiflag;
	...
	void Start () {
		...
		black = GameObject.Find ("BlackLight_hanntenn");
	...
	void Update () {
		...
		dennryoku.transform.localScale = new Vector3 (maxdennryoku, 1f, 1f);
		if (touchCount % 2 == 0) {
			...
			kurayami.gameObject.SetActive (false);
			if (hanntoumeiflag == 1) {
